...
var thelinkarch *obj.LinkArch = &obj.Linkarm64
...
func main(argc int, argv [XXX]string) {
	...
	if !strings.HasPrefix(p, thestring) {
		log.Fatalf("cannot use %ca with GOARCH=%s", thechar, p)
	}

	ctxt = obj.Linknew(thelinkarch)
	ctxt.Diag = yyerror
	ctxt.Bso = &bstdout
	obj.Binit(&bstdout, 1, OWRITE)
	obj.Listinit7()
	...
	flagparse(&argc, (**string)(&argv), usage)
...
func assemble(file string) int {
	...
	ofile = file
	p = utfrrune(ofile, uint(pathchar()))
	if p != "" {
		include[0] = ofile
		p = ""
		p = p[1:]
	...
	if p != "" {
		...
		if systemtype(Plan9) != 0 {
			incfile = fmt.Sprintf("/%s/include", thestring)
			setinclude(incfile)
		}
	}
